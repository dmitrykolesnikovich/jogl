...
public class PerfVBOLoad extends PerfModule {
    ...
    protected void runOneSet(final GLAutoDrawable drawable, final int dataType, final int numObjs, final int numVertices, final int loops, final boolean useVBO) {
        ...
        final int verticesElements = vertices[0].getElementCount() * numObjs;
        final int verticesBytes    = verticesElements * vertices[0].getComponentSizeInBytes()* vertices[0].getComponentCount();
        final int colorsElements   = colors[0].getElementCount()   * colors.length;
        final int colorsBytes      = colorsElements * colors[0].getComponentSizeInBytes()* colors[0].getComponentCount();
